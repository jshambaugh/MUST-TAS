<?php
	/*
		This page queries the timelogs database to find all currently open timelogs, then displays them.
		1. display table to show the results of this query - select profiles.sysid, fname, lname, in24, datein, loginspot, timelogs.status, profiles.status from profiles, timelogs where profiles.sysid = timelogs.sysid and profiles.status = 'active' and timelogs.datein = '$todaysdate' and timelogs.status = 'open' order by loginspot, in24;
		2. get location names from the locations database
	*/
	session_start();
      ...
      <!-- Main Section -->
			...
			<table class="table table-hover">
				...
				<?php
					...
					//execute huge query to find currently logged in users
					$query="select profiles.sysid, fname, lname, in24, datein, loginspot, timelogs.status, profiles.status from profiles, timelogs where profiles.sysid = timelogs.sysid and profiles.status = 'active' and timelogs.datein = '$date' and timelogs.status = 'open' order by loginspot, in24";
					$result=$mysqli->query($query) or die($mysqli->error.__LINE__);
					...
					{
						?> <tr class="error">
										<td>There are no active logins at this time</td>
										<td></td>
										<td></td>
										<td></td>
										<td></td>
					...
					//now, systematically display all relevant information, with profile hyperlinks
					while($obj = $result->fetch_object())
					{
