      ...
      <!-- Main Section -->
		...
		<?php
			//find today's date and time, store in database format
				...
				$time = date("H:i");
		?>		
			<h3>Purge Database started: <?php print("$date at $time <br>"); ?><h3> 
		...
		<?php	
			...
			$TotalRec=$numP+$numR+$numT;
	
			print("There are $TotalRec total database records before purge.<br>");
			
//			$query="call $db_name.DBCleanup()";
//			$result=$mysqli->query($query) or die($mysqli->error.__LINE__);
		?>
...
/*			$query="SELECT * FROM Profiles";
			...
			print("There are $TotalRec total database records after purge.");
*/			
			print("This function is under development.<br>");
		?>
